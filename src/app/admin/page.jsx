...
export default function AdminPage() {
  ...
  const [expansion, setExpansion] = useState(""); // Changed: Default to empty string for blank
  ...
  // States for Expansion selection (pill style)
  ...
  // --- Handlers for managing multiple addon fields (unchanged) ---
  ...
  // --- Handlers for Recommended Classes field (unchanged) ---
  const handleClassInputChange = (e) => {
    ...
    if (value.length > 0) {
      ...
      setFilteredClassSuggestions(allAvailableTags);
      setShowClassSuggestions(true);
    }
  };
  ...
  // --- Handlers for Expansion field ---
  const handleExpansionInputChange = (e) => {
    const value = e.target.value;
    setNewExpansionInput(value); // Update the input field state
  ...
  const handleExpansionInputKeyDown = (e) => {
    if (e.key === "Enter") {
      ...
      if (matchedExpansion) {
        setExpansion(matchedExpansion); // Set the single selected expansion
      ...
      // Allow clearing selected expansion with backspace if input is empty
      setExpansion("");
      setNewExpansionInput("");
      setFilteredExpansionSuggestions([]);
      setShowExpansionSuggestions(false);
  ...
  const handleSelectExpansionSuggestion = (suggestion) => {
    setExpansion(suggestion); // Set the selected expansion
    setNewExpansionInput("");
    setFilteredExpansionSuggestions([]);
    setShowExpansionSuggestions(false);
  };
    ...
    try {
      const response = await fetch("/api/guides", {
        ...
        body: JSON.stringify({
          title,
          category,
          expansion, // Use the selected expansion
          description,
      ...
      setExpansion(""); // Reset to empty string
      setNewExpansionInput("");
      setFilteredExpansionSuggestions([]);
      setShowExpansionSuggestions(false);
      setDescription("");
    ...
    <div className="page-background">
      <div className={styles.container}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          {/* Expansion Input Section (Pill Style) */}
          <div className={styles.formGroup} style={{ position: "relative" }}>
            <label htmlFor="expansionInput">Expansion</label>
            {expansion ? (
              <div
                className={styles.tagPill} // Reusing tagPill style
                style={{
          ...
          {/* Recommended Classes Input Section */}
          <div className={styles.formGroup} style={{ position: "relative" }}>
